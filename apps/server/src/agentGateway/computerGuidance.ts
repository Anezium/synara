...
export const COMPUTER_HELP_SECTIONS = {
  browser:
    'computer_browser_* reaches a browser through the desktop driver\'s CDP route — the only surface that can drive a driver-launched isolated Chromium or an approved existing profile, and the only input route that works on a browser renderer the user is not looking at. Use the integrated browser_* tools for ordinary in-app browser work. Prepare with computer_browser_prepare (allow_launch:true plus an isolated profile): the driver-owned launch is headless by default — no window, no Dock entry — and windowed:true is the opt-in for a visible one. Prefer profile.mode "isolated_named" for multi-step work: it survives the browser restarting, where "isolated_new" starts empty. Bind with computer_browser_state using the prepared pid alone (binding_route "driver_owned_headless"); pid + window_id is only for a native browser window. Snapshot a tab before acting on it. To find something on a page, stay in the browser: computer_browser_type the query into the page\'s own search box (input_route "dom_event", background-safe), take a fresh computer_browser_state, then click the result ref from its outline — repeat per part of the task. Do not leave the browser to look things up elsewhere: the browser is the tool, the rev-31 snapshot\'s name/price digest answers most of it directly, and computer_browser_navigate with the full URL is the fallback, not the first move. To drive a named Chromium-family browser the user already runs (Helium, Chrome, Edge and the like), pass that running browser\'s pid to computer_browser_prepare with allow_launch:true and an isolated profile: the driver launches a headless, driver-owned isolated instance with its own CDP endpoint and never touches the running browser or its profile. target_id (bt-…) names the bound browser; each tab\'s tab_id (tab-…) names one tab inside it — never pass one for the other, and an omitted tab_id resolves only when the target has a single tab or a single active one. Refs are exact and die on navigation — after computer_browser_navigate, take a fresh snapshot instead of retrying stale refs. A status:"refused" result is an answer, not an error: branch on its refusal code (for example browser_requires_setup → prepare, browser_wrong_target_refused → rebind the real window) rather than replaying the call.',
  menus:
...
export function computerToolInstructions(): string {
  return [
    ...
    "For computer_target_ambiguous narrow the target; for stale or missing targets observe again. same_pid_keyboard_ambiguity means background keys cannot prove the window — use an exact element action or foreground, never replay; element_outside_target_window means the ref went stale — fresh get_state and re-address; background_unavailable means that surface takes no background input — use foreground or the browser CDP route. foreground_not_requested means the task never asked to see that app or window — stay in background; confirm with the user before any raise, and their reply that asks to see the screen authorizes it. foreground_user_interaction means the user was just interacting with the desktop — wait for quiet, then retry only if the task still needs visible use. browser_requires_setup means no owned browser endpoint exists — prepare with allow_launch and an isolated profile (approval-gated), after reading the browser chapter; never work around it with coordinate clicks on another window. browser_tab_required/browser_tab_not_found means the tab id is missing or wrong — pass a tabs[].tab_id from the bind result, never target_id. foreign_process_termination_denied means standard mode cannot kill that process — quit cooperatively through its menu or Command-Q, never retry kill. input_target_unavailable means no live input target resolves — observe again with computer_get_state and re-address, never replay. computer_controlled_by_other_thread means shared pointer or focused-keyboard work must wait; an independently addressed exact-window semantic text write may still proceed. When input is paused, stop mutations and hand back to the user; check window readiness after return. Missing permission or ComputerApprovalRequired needs user attention. Only effect=not-dispatched proves no input; effect=dispatched-unknown means inspect and never blindly replay, never automatically retry an unknown effect, and never escalate it to foreground. No consent block this turn means this turn is not yet approved (perception-only calls and already-decided turns never show one). Say exactly that; do not claim control is off.",
    "### The detailed playbook",
    "computer_help lists the chapters this block omits: browser, menus, hidden, foreground, forms, recording. Read the matching topic before touching that surface.",
  ].join("\n");
}
