...
export const COMPUTER_HELP_SECTIONS = {
  ...
  electron:
    "Chromium/Electron often expose several windows in one pid, so background press_key and physical type_text may refuse with same_pid_keyboard_ambiguity. Observe computer_get_state for the exact window_id, then use type_text with a writable ref or label/role; its semantic route can overlap across different windows. set_value replaces the whole field and holds the desktop lease. Neither sends keydown/keyup events: verify autocomplete and submission separately. Use advertised element actions for buttons. Do not retry keys or activate the app without the user's visible-use request.",
  calculator:
...
export function computerToolInstructions(): string {
  return [
    ...
    "### Background first",
    'Prefer semantic controls: pixel fallback can briefly take keyboard focus without raising, then restore it. Visible use requires that the user\'s own latest message asked to watch ("show me the browser", "use foreground mode"); naming an app is insufficient. computer_activate_window, computer_invoke_menu, delivery_mode:"foreground", launch_app hidden:false and browser_prepare windowed:true otherwise refuse with foreground_not_requested. foreground_user_interaction means wait for quiet. Never raise to bypass a background refusal.',
    "### Verdicts and refusals",
    'delivery.effect: "verified" proves an observed effect; "dispatched-unknown" means inspect before deciding, never replay it or escalate to foreground; "not-dispatched" permits a corrected call. same_pid_keyboard_ambiguity is common on multi-window Chromium/Electron: use exact-element type_text, set_value or an advertised action, not more keys. computer_controlled_by_other_thread: another task owns desktop input; wait for its turn to end, do not retry-loop. element_outside_target_window, stale targets or input_target_unavailable: get_state and re-address. repeated_unverified_action: repeated dispatched input had no observed change; change approach. When input is paused, stop and hand back to the user.',
    "### Browser",
