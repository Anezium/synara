...
- The first native release attempt failed macOS icon staging: the macOS 14 runner's older `actool` returned success without creating `Assets.car`. Release runners now select Xcode 16.4 for native code and Xcode 26.3 only for Icon Composer compilation on macOS 15. The catalog remains required; no icon fallback or signing gate was weakened.
...
- Packaging follow-ups passed formatting, lint, typecheck, release smoke and all 231 release-script tests. A real local Icon Composer compile with Xcode 27 produced `Assets.car`; the selected Xcode 26.3 CI build remains the artifact qualification gate.
