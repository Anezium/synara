...
| `mouse_button_down`, `mouse_button_up`                         | Not implemented. The installed rev-15 driver does not admit them through the Synara host allowlist, so no backend method exists to expose. |
| Raw `set_config`, `set_agent_cursor_*`, `get_config`           | Internal only. Provider-controlled driver configuration and cursor ownership would bypass Synara's preview and consent model.              |
