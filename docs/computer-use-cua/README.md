...
- Driver: `cua-driver 0.28.2`, source `fc188250b4ca8549b8e61f937fdb1fb560770e86` (rebased from `0.24.0` / `4b3396d9`; staged native revision 16).
...
| Drag                            | Exact-window target with both endpoints inside its bounds, bounded to ten seconds. Background delivery rides the driver's window-local press-drag-release on drivers that admit it; others refuse `background_unavailable` as `not-dispatched` and explicit foreground mode remains the fallback. Native release guards cover drag; the owned foreground fixture verifies Stop after real movement, matching mouse up and no later movement.                                                                                                                    |
| Scroll                          | Both axes in one gesture, optional held modifiers; requests quantize to Cua's 120-pixel wheel notches, at most 50 per axis per dispatch, posted as pixel-unit deltas. An unmodified vertical scroll on an element target prefers AX scrollbar presses before the wheel. macOS measures travel before and after injection and pre-corrects later requests by learned per-window and per-app gearing. TextEdit-class NSScrollView ignores horizontal wheel deltas (they need continuous trackpad deltas). No corrective replay inferred from uncertain read-back. |
| Cursor                          | Agent cursor overlay; it does not promise a real hover or independent human focus. A real background hover is not deliverable: macOS drops posted pointer moves unless the user's own cursor is inside the target window ([verdict](hover-verdict-2026-09-17.md)).                                                                                                                                                                                                                                                                                              |
...
| Menus / frames / app kill       | `computer_invoke_menu`, `computer_set_window_frame`, and `computer_kill_app` are approval-gated exact-window mutations with second-app consent. A frame reports `verified` only on an independent `list_windows` read-back; a kill only once the pid leaves `list_apps`.                                                                                                                                                                                                                                                                                        |
| Apps / verify / zoom            | `computer_list_apps`, `computer_verify_state`, and `computer_zoom` are read-only. Verify is tri-state (`satisfied`/`unsatisfied`/`unknown`); unknown is not a failure. Zoom returns a display-only JPEG that is never a coordinate frame.                                                                                                                                                                                                                                                                                                                       |
| Hidden workspaces               | `computer_launch_app({hidden:true})`, `computer_set_window_minimized`, and `computer_set_app_visibility` are approval-gated lifecycle mutations that never activate, focus, or switch Spaces. `verified` requires the driver's own AXHidden/AXMinimized read-back evidence; hidden apps and windows stay addressable for semantic actions while coordinate input still needs a visible target.                                                                                                                                                                  |
