...
4. The spawned agent's session carried `mcp__synara__computer_*` tools. Agent
   transcript: *"let me check the overflow file for the `computer_list_windows`
   tool schema"* → *"Calling `computer_list_windows` on the synara MCP server
   now."*
...
- Each individual computer action still requires per-action approval; the
  integration scope only makes the tools *visible* to the task's agent.
