...
Suggested send order: A → B → C → D (increasing size and review burden).
D is the highest-value perf change but the heaviest review; landing A–C
first establishes the relationship and exercises upstream's review process
on low-risk diffs.
...
- `git apply --check`: all four apply cleanly against the pinned base.
- `cargo check`: `-p cua-driver -p platform-macos` clean with A+B+C applied
  ...
  `-p platform-macos` clean with D applied.
- `cargo test -p platform-macos`: `extended_keymap_names_resolve_*` passes
  ...
  `#[ignore]`d — it needs a real application target).
